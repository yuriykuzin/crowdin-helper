...
async function downloadTranslations(shouldIgnoreUnmergedMaster = false) {
  ...
  return await CrowdinApi.getAllTranslations(crowdinBranchName)
    .then(res => {
...
async function uploadSources(shouldExitIfSourceFileWasNotChanged = false) {
  ...
  const isBranchNewlyCreated = await CrowdinApi.addDirectory

  CrowdinApi.addBranch(crowdinBranchName)
...
async function triggerAutoTranslation() {
  ...
  await CrowdinApi.preTranslate(sourceFilesWithBranchName)
    .then((json) => {
      if (!json.success) {
...
async function deleteOldBranches() {
  ...
  crowdinBranches.forEach((branch) => {
    ...
    if (
      ...
      CrowdinApi.deleteBranch(branch.name)
        .then(() => {
